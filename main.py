# Save this code in a file, e.g., simple_http_server.py
"""
...
class SlavePool:
    def __init__(self):
        # Initialize the slave pool with IP addresses
        self.slave_pool = [f'192.168.0.{i}' for i in range(101, 111)]
        self.allocated_slaves = {}
        self.pool_mutex = threading.Lock()

    def allocate(self, amount, duration):
        with self.pool_mutex:
            if len(self.slave_pool) >= amount:
                # making new list of the allocated ip's and modifying the slave_pool list
                allocated = [self.slave_pool.pop(0) for _ in range(amount)]
                for slave in allocated:
                    self.allocated_slaves[slave] = time.time() + duration
                return allocated

    def return_finished_slaves(self):
        with self.pool_mutex:
            current_time = time.time()
            returned_slaves = []
            # Iterate over the allocated_slaves dictionary
            for ip, end_time in self.allocated_slaves.items():
            ...
            for ip in returned_slaves:
                self.allocated_slaves.pop(ip)
                self.slave_pool.append(ip)

    def return_wait_time(self, amount):
        with self.pool_mutex:
            if self.allocated_slaves:
                current_time = time.time()
                remaining_times = []
                # Iterate over the allocated_slaves dictionary
                for _, expiry in self.allocated_slaves.items():
...
# HTTP server class
class SlaveHTTPServer(BaseHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        self.slave_pool = SlavePool()
        super().__init__(*args, **kwargs)

    def do_GET(self):
        # return finished slaves to the pool
        self.slave_pool.return_finished_slaves()
        ...
        allocated_slaves = self.slave_pool.allocate(amount, duration)
        if allocated_slaves:
            response = {"slaves": allocated_slaves}
            logging.info(f"Allocated slaves: {allocated_slaves}")
        else:
            wait_time = self.slave_pool.return_wait_time(amount)
...
if __name__ == '__main__':
    ...
    with socketserver.TCPServer(("", port), SlaveHTTPServer) as httpd:
